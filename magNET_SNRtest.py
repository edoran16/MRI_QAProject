...
show_graphical = True  # display image processing steps
...
with os.scandir(directpath) as the_folders:
    for folder in the_folders:
        ...
        if re.search('-SNR_', fname):
             if re.search('_TRA_', fname):
                if re.search('_NICL', fname):
                    if not re.search('_REPEAT', fname) and not re.search('_PR', fname):
                        print('Loading ', '_TRA_', 'geometry, with ', '_NICL_', 'phantom', 'for', 'FloodField', 'test object')
                        folder = fname
...
bROI1 = mask.copy()  # initialise image matrix for each corner ROI
bROI2 = mask.copy()
bROI3 = mask.copy()
bROI4 = mask.copy()
bROI5 = mask.copy()
...
bROI5[mid_row2-30:mid_row2-10, max_col-10:max_col+10] = 255  # bottom left
